...
/**
 * In ticker, each character in the rendered text is represented by a {@link JyTickerColumn}. The
 * column can be seen as a column of text in which we can animate from one character to the next
 * by scrolling the column vertically. The {@link JyTickerColumnManager} is then a
 * manager/convenience class for handling a list of {@link JyTickerColumn} which then combines into
 * the entire string we are rendering.
 *
 */
class JyTickerColumnManager {
    final ArrayList<JyTickerColumn> mJyTickerColumns = new ArrayList<>();
    private final JyTickerDrawMetrics metrics;
    ...
    JyTickerColumnManager(JyTickerDrawMetrics metrics) {
        this.metrics = metrics;
    }

    /**
     * @see {@link JyTickerView#setCharacterList(char[])}.
     */
    ...
    boolean shouldDebounceText(char[] text) {
        ...
        if (newTextSize == mJyTickerColumns.size()) {
            for (int i = 0; i < newTextSize; i++) {
                if (text[i] != mJyTickerColumns.get(i).getTargetChar()) {
                    return false;
                }
            }
            return true;
    ...
    void setText(char[] text) {
        ...
        for (int i = 0; i < mJyTickerColumns.size(); ) {
            final JyTickerColumn JyTickerColumn = mJyTickerColumns.get(i);
            if (JyTickerColumn.getCurrentWidth() > 0) {
                i++;
            } else {
                mJyTickerColumns.remove(i);
            }
        }

        // Use Levenshtein distance algorithm to figure out how to manipulate the columns
        final int[] actions = JyLevenshteinUtils.computeColumnActions(getCurrentText(), text);
        int columnIndex = 0;
        int textIndex = 0;
        for (int i = 0; i < actions.length; i++) {
            switch (actions[i]) {
                case JyLevenshteinUtils.ACTION_INSERT:
                    mJyTickerColumns.add(columnIndex,
                            new JyTickerColumn(characterList, characterIndicesMap, metrics));
                case JyLevenshteinUtils.ACTION_SAME:
                    mJyTickerColumns.get(columnIndex).setTargetChar(text[textIndex]);
                    columnIndex++;
                    textIndex++;
                    break;
                case JyLevenshteinUtils.ACTION_DELETE:
                    mJyTickerColumns.get(columnIndex).setTargetChar(JyTickerUtils.EMPTY_CHAR);
                    columnIndex++;
                    break;
                default:
    ...
    void onAnimationEnd() {
        for (int i = 0, size = mJyTickerColumns.size(); i < size; i++) {
            final JyTickerColumn column = mJyTickerColumns.get(i);
    ...
    float getMinimumRequiredWidth() {
        float width = 0f;
        for (int i = 0, size = mJyTickerColumns.size(); i < size; i++) {
            width += mJyTickerColumns.get(i).getMinimumRequiredWidth();
        }
        return width;
    }
    ...
    float getCurrentWidth() {
        float width = 0f;
        for (int i = 0, size = mJyTickerColumns.size(); i < size; i++) {
            width += mJyTickerColumns.get(i).getCurrentWidth();
        }
        return width;
    }

    char[] getCurrentText() {
        final int size = mJyTickerColumns.size();
        final char[] currentText = new char[size];
        for (int i = 0; i < size; i++) {
            currentText[i] = mJyTickerColumns.get(i).getCurrentChar();
        }
        return currentText;
    }
